...
ggplot(temp_df) +
  geom_raster(aes(x = x, y = y, fill = `t2m_valid_time=741484800`)) +
...
ggplot() +
  ...
  geom_raster(data = temp_df, aes(x = x, y = y, fill = `t2m_valid_time=741484800`)) +
  scale_fill_viridis_c() +
...
# Let's rename and prepare
info <- na.omit(info)
names(info)[1] <- "poly_id"  # polygon id
...
# check_weights
# 
# # Build sparse transformation matrix (cells × polygons)
# P_weight <- Matrix::sparseMatrix(
#   i = info$cell,
#   j = info$poly_id,
#   x = info$w_norm,
#   dims = c(ncell(r), length(unique(info$poly_id)))
# )
# 
