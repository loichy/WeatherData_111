...
```{r projection-matrix, eval=FALSE}
...
# ID (renamed to poly_id) corresponds exactly to the row order of the polygon object in nuts3_sp
...
# Store as a rds object the P_weight matrix for future use
saveRDS(P_weight, file = here(dir$intermediary, "P_weight_matrix.rds"))
...
Now we have the projection matrix, we can do thespatial aggregation:
...
# Load P_weight matrix if needed
P_weight <- readRDS(here(dir$intermediary, "P_weight_matrix.rds"))
A_weight <- t(P_weight) %*% G

# Convert to data frame
nuts3_temp <- as.data.frame(as.matrix(A_weight))
# create variable which is the polygon identifier: row number
nuts3_temp$poly_id <- 1:nrow(nuts3_temp)

# Have a look at data at nuts-3 level
head(nuts3_temp)
...
## We first need to merge the datasets to have both polygons and temperature data
#  Add the poly_id variable to the nuts3 shapefile
nuts3_sp$poly_id <- 1:nrow(nuts3_sp)

# Merge aggregated temperature data using poly_id
nuts3_sp <- merge(nuts3_sp, nuts3_temp, by = "poly_id", all.x = TRUE)


# pUT NExt to each others temperature a raster-cell levels and temperature at nuts3 level to compare if temperature corresponds:
# Plot raster temperature
par(mfrow=c(1,2))  # set up the plotting area to have 2 plots side by side
plot(terra_temp[[1]], main = "Raster Temperature (ERA5)", col = terrain.colors(100))
# Plot nuts3 temperature
plot(nuts3_sp["t2m_valid_time=741484800"], main = "NUTS3 Aggregated Temperature", col = terrain.colors(100))
...
There is a mistage here in the way we identify the raster -> need to update this
...
table(gdp_nuts3$unit)
# Common units:
# - MIO_EUR: Million real euros
...
unique(gdp_nuts3_current$TIME_PERIOD)


# View the result
head(gdp_nuts3_current)

# Plot some of the GDP series to have a look
# Select randomly 10 NUTS3 regions
set.seed(123)
sample_nuts3 <- sample(unique(gdp_nuts3_current$geo), 10)
gdp_nuts3_filter <- gdp_nuts3_current %>%
  filter(geo %in% sample_nuts3)

# plot their time series
ggplot(gdp_nuts3_filter, aes(x = TIME_PERIOD, y = values, color = geo)) +
  geom_line() +
  labs(title = "GDP Time Series for Sample NUTS3 Regions",
       x = "Year",
       y = "GDP (Million Euros)") +
  theme_minimal()
